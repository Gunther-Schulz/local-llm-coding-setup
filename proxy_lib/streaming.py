...
def process_stream_end(state: StreamState) -> Generator[str, None, None]:
    ...
    if state.accumulated_content and should_transform_tool_calls(state.vllm_sent_tool_calls):
        ...
        if DEBUG_MODE and tool_calls:
            format_msg = {
                "qwen2.5": "Always transform",
                "qwen3": "vLLM failed, fallback",
                "auto": "Auto-detected"
            }.get(os.environ.get("MODEL_TOOL_FORMAT", "auto"), "Transform")
            print(f"[DEBUG] {format_msg}: {len(tool_calls)} tool call(s)")
