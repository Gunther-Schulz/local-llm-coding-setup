...
"$ROOT/run_server.sh" "${CODE_VISION_VISION_MODEL:?CODE_VISION_VISION_MODEL not set}" "$VISION_PORT" &
VISION_PID=$!
trap "kill $VISION_PID 2>/dev/null || true" EXIT
sleep 2
echo "Starting coding server on $CODING_PORT (foreground)..."
exec "$ROOT/run_server.sh" "${CODE_MODEL:?CODE_VISION_CODING_MODEL or CODING_MODEL not set}" "$CODING_PORT"
